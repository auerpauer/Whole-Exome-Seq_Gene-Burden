...
class MutationCount:
    ...
    def __init__(self, vcfFilename, outputDir, excel):
        ...
        self.countingSets = {} # Hash that uses gene name, variant type and zygosity (het/hom) to point
                               # to sample names that share all three of these values.
        return
    ...
    def process_oldest_gene(self):
        ...
        # count compound heterozygotes across variant types
    ...
    def write_to_tables(self, geneName, varCount, smpCount):
        ...
        # These have already been added together
...
#######################
##### ExomeBurden #####
# Each line in the VCF file is processed independently. The only exception to this is counting compound heterozygous variants.
#
...
# extra code. The most egregious of these is that there are two different names for ExAC data, used by different VCF
...
# the elements in the list are matched against values in the VCF file. Once a match is found, that value is used to overwrite
...
# This means that, for each sample, each variant is written out in tab-delimited format. This creates a very great redundancy in the
...
class ExomeBurden:
    ...
    # Please note that all lists that count variants are in the same order as this one
    allowedVariantTypes = ('missense', 'deleterious', 'lof')
    # hardcoded indexes into 'mutationCount'
    het = 0
    hom = 1
    # possible genotypes in sample data that we do not want to process (they don't contain any trackable variants)
    ...
    def read_variants(self):
        ...
        while line:
            ...
            prefixHash = self.populate_allele_list(altAlleles.split(','), infoField) # used in 'melt_vcf_record()'

            ### DEBUG
            #if chrom == '7' and position == '142583330': pdb.set_trace()
            if 'ZNF888' == self.alleleList[index][1]['Gene.refGene']:
                DEBUG_FLAG = True
            ### END DEBUG
            ...
            while index < len(self.alleleList):
                ...
                try:
                    ...
                    if exc.errCode > BurdenException.filtered_out:
                        if DEBUG_FLAG:
                                sys.stderr.write('DEBUG: caught exception: {0}\n'.format(exc.errCode))
                        index += 1
                        continue
                    else:
            ...
            if allFail:
                ...
                if DEBUG_FLAG:
                    sys.stderr.write('DEBUG: fail refgene values\n')
            else:
