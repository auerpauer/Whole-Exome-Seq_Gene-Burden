...
This program writes a tab-delimited file that contains, for each gene, the number of counts for cases and controls and their p-value for each variant category described above.

At this point, you need to decide on a q-value threshold. You must use q-value, because we are doing multiple tests against all human genes. This is true, even if only a subset of these genes appear in the file produced by the gene_burden_fisher_exact.py program. The threshold we used was 2.5E-6. This was achieved by dividing 0.05 (a normal p-value threshold) by 20,000 (the approximate number of human genes).
